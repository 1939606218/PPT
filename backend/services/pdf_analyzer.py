...
# 并发分析幻灯片时的最大同时请求数
# 实测魔塔社区 VL 模型极度受限（串行连续请求也会 429），必须串行+间隔
VL_CONCURRENCY = 1
# 每次 VL 请求成功后等待的最小间隔（秒），避免触发 RPM 限制
VL_REQUEST_INTERVAL = 3.0
...
class PDFAnalyzer:
    ...
    async def _analyze_slides(self, images_b64: List[str], progress_cb=None) -> List[Dict]:
        """串行调用 VL 模型分析幻灯片（魔塔 VL 模型 RPM 极低，必须串行+间隔）"""
        semaphore = asyncio.Semaphore(VL_CONCURRENCY)
        total = len(images_b64)
        completed = 0
        _last_vl_time: list[float] = [0.0]  # 用列表以便闭包修改

        async def analyze_one(i: int, b64: str) -> Dict:
            nonlocal completed
            async with semaphore:
                # 距上次请求不足 VL_REQUEST_INTERVAL 秒则等待
                elapsed = asyncio.get_event_loop().time() - _last_vl_time[0]
                if elapsed < VL_REQUEST_INTERVAL:
                    await asyncio.sleep(VL_REQUEST_INTERVAL - elapsed)
                page_no = i + 1
                ...
                for attempt in range(1, 4):
                    try:
                        ...
                        _last_vl_time[0] = asyncio.get_event_loop().time()
                        ...
                        # 429 限流：指数退避；其他错误：固定等待
                        wait = (2 ** attempt) * 5 if is_rate_limit else 1.0
                        logger.warning(
                            f"  第{page_no}页VL分析第{attempt}次失败: {e}"
                            + (f"，{'限流' if is_rate_limit else ''}重试中（等待{wait:.0f}s）..." if attempt < 3 else "，进入清洗阶段")
                        )
                        if attempt < 3:
                ...
                if slide_info is None:
                    if raw is not None:
                        try:
                            slide_info = self._parse_json_response(raw)
                            slide_info["page_number"] = page_no
                        ...
                        logger.warning(f"  第{page_no}页VL分析彻底失败（无响应），使用默认值")
                        slide_info = self._default_slide_info(page_no)
