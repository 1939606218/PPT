...
# ── 全局并发控制：同一时刻只允许 1 个评分任务执行（API 限速决定） ──────────
# 超出的请求会在信号量处等待，不会报错，前端通过 queue_position 感知排队
_analyze_semaphore = asyncio.Semaphore(1)
_analyze_queue_count = 0          # 当前排队等待数（不含正在执行的）
...
async def analyze_presentation(
    ...
    global _analyze_queue_count

    # ── 全局排队控制：超过 5 人排队直接拒绝，避免无限堆积 ──────────────────
    MAX_QUEUE = 5
    if _analyze_queue_count >= MAX_QUEUE:
        raise HTTPException(
            status_code=429,
            detail=f"服务繁忙，当前已有 {_analyze_queue_count} 人排队，请稍后再试"
        )

    _analyze_queue_count += 1
    try:
        # 等待信号量（同一时刻只有 1 个任务真正执行）
        async with _analyze_semaphore:
            _analyze_queue_count -= 1
            return await _do_analyze(pdf_file, audio_file, db, current_user)
...
async def get_queue_status():
    """返回当前排队状态，供前端在提交前提示用户"""
    return {
        "busy": _analyze_semaphore.locked(),
        "queue_count": _analyze_queue_count,
        "message": f"当前 {_analyze_queue_count} 人排队等候" if _analyze_queue_count > 0 else (
            "服务繁忙，处理中" if _analyze_semaphore.locked() else "空闲，可立即提交"
        ),
    }
