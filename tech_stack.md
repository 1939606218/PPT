...
| 模型名称     | 输入（元 / 百万 Tokens） | 输出（元 / 百万 Tokens） |
| :----------- | :----------------------: | :----------------------: |
| Qwen3-Max    |           2.5           |            10            |
| Qwen-VL-Plus |           0.8           |            2            |
...
flowchart TD
    ...
    subgraph 线路1["线路1：视觉理解"]
        ...
        SUM3["qwen3-max<br/>输入：所有页面描述摘要<br/>输出①：布局质量 JSON<br/>输出②：内容结构 JSON<br/>输出③：视觉元素 JSON"]
        LO --> IMG --> VL --> SUM3
    end
    ...
    subgraph S2["② 分类 LLM"]
        CLS["qwen3-max<br/>输入：完整上下文<br/>输出：PPT 类型（产品创新 / 问题解决 / 降本增效 / 方法工具）"]
    end
    MERGE --> CLS

    subgraph S3["③ 并行评分 LLM（3个同时）"]
        direction LR
        DA["qwen3-max<br/>输入：类型 + 上下文<br/>输出：维度A得分（0–45）+ 点评"]
        DB["qwen3-max<br/>输入：类型 + 上下文<br/>输出：维度B得分（0–45）+ 点评"]
        DC["qwen3-max<br/>输入：类型 + 上下文<br/>输出：维度C得分（0–10）+ 点评"]
    end
    CLS --> DA & DB & DC

    subgraph S4["④ 汇总 LLM"]
        SUMM["qwen3-max<br/>输入：3个维度得分 + 点评<br/>输出：总分 / 优点 / 不足 / 改进建议"]
    end
    DA & DB & DC --> SUMM
...
1. **双线路文本提取**
   - 线路1（视觉理解）：.pptx/.ppt文件经LibreOffice headless转为PDF后，通过PyMuPDF生成PNG截图列表；调用qwen-vl-plus逐页分析（并发5），提取文字内容与图表描述；再由qwen3-max汇总为布局质量、内容结构、视觉元素三类JSON数据。
   - 线路2（音频转录）：音频文件通过AssemblyAI API自动识别语言，完成转录，输出完整文本与词级时间戳。
2. **上下文合并**：将视觉分析结果与音频转录文本融合，生成包含PPT页面内容、语音全文、语速及停顿指标的完整上下文。
3. **智能分类**：qwen3-max接收合并后的上下文，精准判定PPT类型（产品创新/问题解决/降本增效/方法工具）。
4. **多维度并行评分**：3个qwen3-max模型同步运行，分别从3个维度（A/B/C）输出0-45分、0-10分不等的得分及针对性点评，提升评估全面性。
...
| 模块名称             | 模型名称     | 输入Tokens | 输出Tokens | 思考Tokens | 合计Tokens       |
| -------------------- | ------------ | ---------- | ---------- | ---------- | ---------------- |
...
| VL-汇总分析          | qwen3-max    | 1363       | 152        | -          | 1515             |
| 分类LLM              | qwen3-max    | 3530       | 140        | 2660       | 6330             |
| 维度A评分LLM         | qwen3-max    | 1693       | 471        | 2609       | 4773             |
| 维度B评分LLM         | qwen3-max    | 3967       | 953        | 3236       | 8156             |
| 维度C评分LLM         | qwen3-max    | 4028       | 934        | 3071       | 8033             |
| 汇总LLM              | qwen3-max    | 4397       | 1679       | 3577       | 9653             |
| **总计**             | -            | -          | -          | **15153**  | **49622**  |

### （四）费用核算明细（汇率1USD = 7.25CNY）

| 费用模块       | 具体说明                                                    | 输入费用  | 输出费用  | 思考费用  | 合计费用           |
| -------------- | ----------------------------------------------------------- | --------- | --------- | --------- | ------------------ |
| VL逐页分析     | qwen-vl-plus（6页，输入8916 / 输出2246 tokens）             | ¥0.0071  | ¥0.0045  | -         | ¥0.0116           |
| VL后处理LLM    | qwen3-max（输入1363 / 输出152 tokens）                      | ¥0.0034  | ¥0.0015  | -         | ¥0.0049           |
| 音频转录       | AssemblyAI（428秒，universal-3-pro）                        | -         | -         | -         | $0.0249 ≈ ¥0.1808 |
| 分类LLM        | qwen3-max（输入3530 / 输出140 / 思考2660 tokens）           | ¥0.0088  | ¥0.0014  | ¥0.0266  | ¥0.0368           |
| 维度A评分LLM   | qwen3-max（输入1693 / 输出471 / 思考2609 tokens）           | ¥0.0042  | ¥0.0047  | ¥0.0261  | ¥0.0350           |
| 维度B评分LLM   | qwen3-max（输入3967 / 输出953 / 思考3236 tokens）           | ¥0.0099  | ¥0.0095  | ¥0.0324  | ¥0.0518           |
| 维度C评分LLM   | qwen3-max（输入4028 / 输出934 / 思考3071 tokens）           | ¥0.0101  | ¥0.0093  | ¥0.0307  | ¥0.0501           |
| 汇总LLM        | qwen3-max（输入4397 / 输出1679 / 思考3577 tokens）          | ¥0.0110  | ¥0.0168  | ¥0.0358  | ¥0.0636           |
| **总费用** | -                                                           | -         | -         | -         | **¥0.4346**  |
